...
let notes_list ~ctx ~cache accept (local_ rctx) (local_ respond) =
  ...
  negotiated ~cache ~key rctx accept
    ~html_fn:(fun () ->
      ...
      C.Layout.page ~ctx ~title:"Notes" ~description:"Notes and blog posts" ~url:"/notes" ~current_page:"Notes" ~jsonld ~page_scripts:[Notes_calendar; Tag_cloud_filter; Pagination; Toc] ~main_cls:"max-w-4xl" ~article ~sidebar ())
