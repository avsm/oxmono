...
let auth_method_jsont =
  let oauth_jsont =
    Jsont.Object.map ~kind:"OAuth"
      (fun access_token refresh_token client_id client_secret ->
        OAuth { access_token; refresh_token; client_id; client_secret })
    |> Jsont.Object.mem "access_token" Jsont.string ~enc:(function
         | OAuth { access_token; _ } -> access_token)
    |> Jsont.Object.opt_mem "refresh_token" Jsont.string ~enc:(function
         | OAuth { refresh_token; _ } -> refresh_token)
    |> Jsont.Object.mem "client_id" Jsont.string ~enc:(function
         | OAuth { client_id; _ } -> client_id)
    |> Jsont.Object.mem "client_secret" Jsont.string ~enc:(function
         | OAuth { client_secret; _ } -> client_secret)
    |> Jsont.Object.finish
  in
  Jsont.Object.map ~kind:"AuthMethod"
    (fun type_ oauth ->
      match type_ with
      | "oauth" -> Option.get oauth
      | _ -> failwith ("Unknown auth type: " ^ type_))
  |> Jsont.Object.mem "type" Jsont.string ~enc:(function
       | OAuth _ -> "oauth")
  |> Jsont.Object.opt_mem "oauth" oauth_jsont ~enc:(function
       | OAuth _ as o -> Some o)
  |> Jsont.Object.finish
