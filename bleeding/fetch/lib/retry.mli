(** Retry policies for {!Fetch.with_retry}. *)

type config = {
  max_retries : int;
      (** [max_retries] is the number of attempts after the first. It defaults
          to 3. *)
  backoff_factor : Duration.t;
      (** [backoff_factor] is the scale of exponential backoff. Retry [n] waits
          [Duration.to_f backoff_factor *. 2. ** float (n - 1)] seconds.
          It defaults to 500 milliseconds. *)
  backoff_max : Duration.t;
      (** [backoff_max] is the maximum delay, including a
          server-supplied [Retry-After] delay. It defaults to 120 seconds. *)
  status_forcelist : int list;
      (** [status_forcelist] is the list of response statuses to retry. It
          defaults to [[429; 500; 502; 503; 504]]. *)
  allowed_methods : Http.Method.t list;
      (** [allowed_methods] is the list of methods that may be retried. The
          default contains the idempotent methods of
          {{:https://www.rfc-editor.org/rfc/rfc9110#section-9.2.2}RFC
          9110 §9.2.2}: GET, HEAD, OPTIONS, PUT, and DELETE. *)
  respect_retry_after : bool;
      (** [respect_retry_after] is [true] when retries honour the
          [Retry-After] response header. It defaults to [true]. *)
  jitter : bool;
      (** [jitter] is [true] when each computed backoff is spread between zero and
          its full delay, drawing from {!Fetch.with_retry}'s [random]. It does
          not affect a server-supplied [Retry-After] delay. It defaults to
          [true]. *)
  retry_exception : (exn -> bool) option;
      (** [retry_exception] is the optional predicate selecting additional
          exceptions to retry, including
          exceptions raised by middleware. It is never called for
          [Eio.Cancel.Cancelled], or for
          [Denied], [Tls_failure], [Body_not_replayable],
          [Invalid_url] and [Invalid_request], which a re-issue cannot
          fix. It defaults to [None]. *)
  retry_response : (Middleware.request -> Middleware.response -> bool) option;
      (** [retry_response] is the optional predicate selecting additional
          responses to retry. It defaults to [None]. *)
