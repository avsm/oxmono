...
let src = Logs.Src.create "fetch.signature" ~doc:"HTTP Message Signatures"
...
(* RFC 9421 2.1: a field with several values is covered by joining them with
   a comma and a single space, each value stripped of leading and trailing
   whitespace. [Http.Header] lookups are case-insensitive. *)
let header_value headers name =
  match Http.Header.get_multi headers name with
  | [] -> None
  | values -> Some (String.concat ", " (List.map String.trim values))

let header_set headers name value = Http.Header.replace headers name value
...
let http_date t =
  ...
  in
  Printf.sprintf "%s, %02d %s %04d %02d:%02d:%02d GMT" wday d mon y hh mm ss
...
module Component = struct
  ...
  type param = [ `Sf | `Key of string | `Bs | `Tr | `Req ]
  ...
  let of_identifier str =
    ...
    let extract_name s =
      ...
      | Some idx ->
          ...
          if String.length rest >= 6 && String.sub rest 0 6 = ";name=" then
            ...
            if String.length after > 0 && after.[0] = '"' then
              ...
              | Some end_idx ->
                  ...
                  let remaining =
                    String.sub s 0 idx
                    ^ String.sub after (end_idx + 1)
                        (String.length after - end_idx - 1)
                  in
                  (remaining, Some name)
              | None -> (s, None)
            else (s, None)
...
module Params = struct
  ...
  let get_tag t = t.tag
  let ptime_to_unix t = Int64.of_float (Ptime.to_float_s t)
...
module Key = struct
  ...
  type t = { priv : private_key option; pub : public_key option }

  let symmetric secret =
    { priv = Some (Symmetric_priv secret); pub = Some (Symmetric_pub secret) }

  let ed25519 ~priv ~pub =
    { priv = Some (Ed25519_priv priv); pub = Some (Ed25519_pub pub) }

  let ed25519_priv priv = { priv = Some (Ed25519_priv priv); pub = None }
  let ed25519_pub pub = { priv = None; pub = Some (Ed25519_pub pub) }
  ...
  let p256_pub pub = { priv = None; pub = Some (P256_pub pub) }
  ...
  let p384_pub pub = { priv = None; pub = Some (P384_pub pub) }
  ...
  let rsa_pub pub = { priv = None; pub = Some (Rsa_pub pub) }
...
module Context = struct
  ...
  let request ~method_ ~uri ~headers : t = `Request { method_; uri; headers }
  ...
  let with_headers (t : t) headers : t =
    match t with
    | `Request r -> `Request { r with headers }
    | `Response r -> `Response { r with headers }
end
...
module Content_digest = struct
  ...
  let add ~algorithm ~body headers =
    ...
    header_set headers "content-digest" value
  ...
  let verify ~header ~body =
    ...
    let extract_b64 rest =
      if String.length rest >= 2 && rest.[0] = ':' then
        ...
        if end_idx > 0 then Some (String.sub rest 1 (end_idx - 1)) else None
      else None
    in
...
let resolve_component (ctx : Context.t) (component : Component.t) =
  let get_headers : Context.t -> Http.Header.t = function
    | `Request r -> r.headers
    | `Response r -> r.headers
  in
  let get_request_headers : Context.t -> Http.Header.t option = function
    | `Request r -> Some r.headers
    | `Response (r : response_ctx) ->
        Option.map (fun (req : request_ctx) -> req.headers) r.request
  in
  match component with
  | `Derived (d, params) -> (
      let has_req = List.mem `Req params in
      let target_ctx : Context.t =
        if has_req then
          match ctx with
      ...
      | `Method -> (
          ...
          | `Request r -> Ok (Http.Method.to_string r.method_)
  ...
  | `Field (name, params) -> (
      let has_req = List.mem `Req params in
      let headers =
        if has_req then get_request_headers ctx else Some (get_headers ctx)
      in
      match headers with
      | None -> Error ("Cannot resolve request-bound field: " ^ name)
      | Some hdrs -> (
          match header_value hdrs name with
...
let build_signature_base ~(components : Component.t list)
    ~(params : Params.t) (ctx : Context.t) =
  let component_lines =
    List.map
      (fun c ->
  ...
  (* Check for errors *)
  let errors =
    List.filter_map (function Error e -> Some e | Ok _ -> None) component_lines
  in
  if errors <> [] then Error (List.hd errors)
  else
    let lines =
      List.filter_map (function Ok l -> Some l | Error _ -> None) component_lines
    in
...
let sign_bytes ~(alg : Algorithm.t) ~(key : Key.t) (data : string) =
  ...
  | `Rsa_v1_5_sha256, Some (Key.Rsa_priv priv) ->
      let hash = Digestif.SHA256.(to_raw_string (digest_string data)) in
      let sig_ =
        Mirage_crypto_pk.Rsa.PKCS1.sign ~hash:`SHA256 ~key:priv (`Digest hash)
      in
      Ok sig_
  | alg, None -> Error ("Missing private key for " ^ Algorithm.to_string alg)
  | _, _ -> Error "Key type mismatch for algorithm"

let verify_bytes ~(alg : Algorithm.t) ~(key : Key.t) ~(signature : string)
    (data : string) =
  ...
  | `Ed25519, Some (Key.Ed25519_pub pub) -> (
      ...
      | Ok pub_key ->
          let valid =
            Mirage_crypto_ec.Ed25519.verify ~key:pub_key signature ~msg:data
          in
  ...
  | `Rsa_v1_5_sha256, Some (Key.Rsa_pub pub) ->
      ...
      let valid =
        Mirage_crypto_pk.Rsa.PKCS1.verify ~hashp ~key:pub ~signature
          (`Digest hash)
      in
...
let sign ~clock ~(config : config) ~(context : Context.t)
    ~(headers : Http.Header.t) =
  if not (Key.can_sign config.key) then Error `Missing_private_key
    ...
    let params =
      Params.empty
      |> (fun p ->
           if config.include_created then Params.created (now_ptime clock) p
           else p)
    ...
    let* base =
      Result.map_error
        (fun e -> `Component_resolution_error e)
        (build_signature_base ~components:config.components ~params context)
    in
    let* sig_bytes =
      Result.map_error
        (fun e -> `Crypto_error e)
        (sign_bytes ~alg ~key:config.key base)
    in
    (* Build headers *)
    ...
    let sig_input_header =
      Sf.dictionary_to_string [ (config.label, Sf.Inner_list sig_input_il) ]
    in
    let sig_header =
      Sf.dictionary_to_string
        [ (config.label, Sf.Item (Sf.Byte_seq sig_bytes, [])) ]
    in
    ...
    Ok
      (headers
      |> fun h ->
      header_set h "signature-input" sig_input_header |> fun h ->
      header_set h "signature" sig_header)

let sign_with_digest ~clock ~(config : config) ~(context : Context.t)
    ~(headers : Http.Header.t) ~(body : string)
  ...
  (* Keep the context in step, so that the content-digest component resolves
     against the header we have just added rather than the caller's original
     block. *)
  let context = Context.with_headers context headers in
  (* Add content-digest to components if not already present *)
  let has_content_digest =
    List.exists
      (function `Field ("content-digest", _) -> true | _ -> false)
      config.components
  in
  let components =
    if has_content_digest then config.components
...
let verify_error_to_string : verify_error -> string = function
  ...
  | `Required_component_missing s ->
      "Required component missing from signature: " ^ s
  | `Crypto_error s -> "Cryptographic error: " ^ s
...
let verify ~clock ~(key : Key.t) ?label ?max_age ?required_components
    ~(context : Context.t) ~(headers : Http.Header.t) () =
  ...
  let check_expiration sig_params =
    ...
    | Some exp when Ptime.is_earlier exp ~than:now ->
        Log.debug (fun m ->
            m "Signature expired: expires=%a now=%a" Ptime.pp exp Ptime.pp now);
  ...
  let check_max_age sig_params =
    ...
    | Some age, Some created -> (
        ...
        | Some limit when Ptime.is_earlier limit ~than:now ->
            Log.debug (fun m ->
                m "Signature too old: created=%a max_age=%a now=%a" Ptime.pp
                  created Ptime.Span.pp age Ptime.pp now);
            Error `Signature_expired
        | _ -> Ok ())
    | _ -> Ok ()
  in
  ...
  let check_not_future sig_params =
    ...
    | Some created when Ptime.is_later created ~than:now -> (
        ...
        | Some limit when Ptime.is_later created ~than:limit ->
            Log.warn (fun m ->
                m "Signature created in the future: created=%a now=%a" Ptime.pp
                  created Ptime.pp now);
  ...
  if not (Key.can_verify key) then Error `Missing_public_key
    ...
    let* sig_header =
      require (header_value headers "signature") `Missing_signature_header
    in
    let* sig_input_header =
      require
        (header_value headers "signature-input")
        `Missing_signature_input_header
    in
    let* sig_dict =
      Result.map_error
        (fun e -> `Invalid_signature_input ("Signature: " ^ e))
        (Sf.parse_dictionary sig_header)
    in
    let* sig_input_dict =
      Result.map_error
        (fun e -> `Invalid_signature_input ("Signature-Input: " ^ e))
        (Sf.parse_dictionary sig_input_header)
    in
    let target_label =
      label
      |> Option.value
           ~default:(match sig_dict with (l, _) :: _ -> l | [] -> "sig1")
    in
    let* sig_member =
      require
        (List.assoc_opt target_label sig_dict)
        (`Signature_label_not_found target_label)
    in
    let* sig_bytes =
      match sig_member with
      | Sf.Item (Sf.Byte_seq s, _) -> Ok s
      | _ -> Error (`Invalid_signature_input "Expected byte sequence")
    in
    let* input_member =
      require
        (List.assoc_opt target_label sig_input_dict)
        (`Signature_label_not_found target_label)
    in
    let* items, params =
      match input_member with
    ...
    let alg =
      match (sig_params.Params.alg, Key.algorithm key) with
      | Some a, _ -> a
      | None, Some a -> a
      | None, None -> `Ed25519
    in
    ...
    let* base =
      Result.map_error
        (fun e -> `Component_resolution_error e)
        (build_signature_base ~components ~params:sig_params context)
    in
    let* () =
      Result.map_error
        (fun e -> `Crypto_error e)
        (verify_bytes ~alg ~key ~signature:sig_bytes base)
    in
    Ok
      {
        label = target_label;
...
let verify_all ~clock ~key_resolver ?max_age ~(context : Context.t)
    ~(headers : Http.Header.t) () =
  match
    (header_value headers "signature", header_value headers "signature-input")
  with
  | None, _ -> Error `Missing_signature_header
  | _, None -> Error `Missing_signature_input_header
  | Some sig_header, Some sig_input_header -> (
      match
        (Sf.parse_dictionary sig_header, Sf.parse_dictionary sig_input_header)
      with
      ...
      | Ok sig_dict, Ok sig_input_dict -> (
          let results =
            List.filter_map
              (fun (label, _) ->
                ...
                let keyid =
                  match List.assoc_opt label sig_input_dict with
                ...
                | Some kid -> (
                    ...
                    | Some key -> (
                        match
                          verify ~clock ~key ~label ?max_age ~context ~headers ()
                        with
                        | Ok r -> Some (Ok r)
          ...
          in
          let successes =
            List.filter_map (function Ok r -> Some r | _ -> None) results
          in
          let errors =
            List.filter_map (function Error e -> Some e | _ -> None) results
          in
