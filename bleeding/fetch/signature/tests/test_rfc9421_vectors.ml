...
(** RFC 9421 Appendix B Test Vectors
    ...
    from {{:https://datatracker.ietf.org/doc/html/rfc9421#appendix-B}RFC 9421
    Appendix B}. *)
...
(** B.1.4 test-key-ed25519
    ...
    The Ed25519 key pair from RFC 9421 Appendix B.1.4. JWK format:
    {[
      {
        "kty": "OKP",
        "crv": "Ed25519",
...
let test_key_ed25519_priv =
  base64url_decode "n4Ni-HpISpVObnQMW0wOhCKROaIKqKtW_2ZYb2p9KcU"

let test_key_ed25519_pub =
  base64url_decode "JrQLj5P_89iXES9-vFgrIy29clF9CC_oPPsw3c5D0bs"
...
(** The test-request from RFC 9421 Appendix B.2:
    ...
    POST /foo?param=Value&Pet=dog HTTP/1.1 Host: example.com Date: Tue, 20 Apr
    2021 02:07:55 GMT Content-Type: application/json Content-Digest:
...
let test_request_headers =
  empty
  |> set "date" "Tue, 20 Apr 2021 02:07:55 GMT"
  |> set "content-type" "application/json"
  |> set "content-digest"
       "sha-512=:WZDPaVn/7XgHaAy8pmojAkGWoRx2UFChF41A2svX+TaPm+AbwAgBWnrIiYllu7BNNyealdVLvRwEmTHWXvJwew==:"
  |> set "content-length" "18"
...
"@signature-params": ("date" "@authority" "content-type");created=1618884473;keyid="test-shared-secret"
    ...
    Expected Signature-Input: sig-b25=("date" "@authority"
    "content-type");created=1618884473;keyid="test-shared-secret" Expected
...
let test_b25_hmac_verify clock =
  ...
  in
  let context =
    Signature.Context.request ~method_:`POST ~uri:test_request_uri ~headers
  in
  match Signature.verify ~clock ~key ~label:"sig-b25" ~context ~headers () with
  | Error e ->
      Alcotest.fail
        ("B.2.5 HMAC verification failed: "
       ^ Signature.verify_error_to_string e)
  | Ok result ->
      Alcotest.(check (option string))
        "keyid matches" (Some "test-shared-secret") result.keyid;
      Alcotest.(check int)
...
let test_b26_ed25519_verify clock =
  ...
  | Error e ->
      Alcotest.fail
        ("B.2.6 Ed25519 verification failed: "
       ^ Signature.verify_error_to_string e)
  | Ok result ->
      Alcotest.(check (option string))
        "keyid matches" (Some "test-key-ed25519") result.keyid;
      Alcotest.(check int)
        "verified 6 components" 6
        (List.length result.verified_components)
...
let test_b4_transform_signature_base () =
  ...
  | Ok priv_key ->
      ...
      in
      Alcotest.(check string) "B.4 Transform Ed25519 signature" expected_sig
        sig_b64

let b4_headers =
  empty
  |> set "date" "Fri, 15 Jul 2022 14:24:55 GMT"
  |> set "signature-input"
       "transform=(\"@method\" \"@path\" \"@authority\" \
        \"accept\");created=1618884473;keyid=\"test-key-ed25519\""
  |> set "signature"
       "transform=:ZT1kooQsEHpZ0I1IjCqtQppOmIqlJPeo7DHR3SoMn0s5JZ1eRGS0A+vyYP9t/LXlh5QMFFQ6cpLt2m0pmj3NDA==:"
...
let test_b4_transform_verify clock =
  ...
  | Error e ->
      Alcotest.fail
        ("B.4 Transform verification failed: "
       ^ Signature.verify_error_to_string e)
  | Ok result ->
      Alcotest.(check (option string))
        "keyid matches" (Some "test-key-ed25519") result.keyid;
      Alcotest.(check int)
        "verified 4 components" 4
        (List.length result.verified_components)

(** B.4 also allows the two [Accept] values to arrive as separate fields, which
    RFC 9421 §2.1 says are combined with a comma and a single space. *)
let test_b4_transform_verify_split_accept clock =
  let key =
    Signature.Key.ed25519 ~priv:test_key_ed25519_priv ~pub:test_key_ed25519_pub
  in
  let uri = Uri.of_string "https://example.org/demo?name1=Value1&Name2=value2" in
  ...
  | Error e ->
      Alcotest.fail
        ("B.4 split-field verification failed: "
       ^ Signature.verify_error_to_string e)
  | Ok _ -> ()

(** Test that modified messages fail verification - B.4 example where method
    changed from GET to POST should fail *)
...
let test_b4_transform_authority_modified_fails clock =
  ...
  | Ok _ ->
      Alcotest.fail
        "B.4 Authority-modified message should have failed verification"
...
let test_http_date () =
  match Ptime.of_date_time ((2021, 4, 20), ((2, 7, 55), 0)) with
  | None -> Alcotest.fail "bad date"
  | Some t ->
      Alcotest.(check string)
        "IMF-fixdate" "Tue, 20 Apr 2021 02:07:55 GMT"
        (Signature.http_date t)
...
let transform_tests =
  [
    ...
    ( "B.4 Transform verify (split accept)",
      `Quick,
      with_clock test_b4_transform_verify_split_accept );
    ("B.4 Modified method fails", `Quick, with_clock test_b4_transform_modified_fails);
    ( "B.4 Modified authority fails",
      `Quick,
      with_clock test_b4_transform_authority_modified_fails );
  ]
...
let date_tests = [ ("IMF-fixdate", `Quick, test_http_date) ]
