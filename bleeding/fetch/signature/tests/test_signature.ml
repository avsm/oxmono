...
module Signature = Fetch_signature

let empty = Http.Header.init ()
let set name value hdrs = Http.Header.replace hdrs name value
let get name hdrs = Http.Header.get hdrs name
...
let test_component_parse () =
  let tests =
    [ ("@method", true); ("@authority", true); ("content-type", true);
...
let test_algorithm_roundtrip () =
  let algs : Signature.Algorithm.t list =
    [ `Ed25519; `Hmac_sha256; `Ecdsa_p256_sha256; `Ecdsa_p384_sha384;
      `Rsa_pss_sha512; `Rsa_v1_5_sha256 ]
  in
  List.iter
    (fun alg ->
...
let test_content_digest_sha256 () =
  ...
  Alcotest.(check bool)
    "Has sha-256 prefix" true
    (String.length digest > 8 && String.sub digest 0 8 = "sha-256=")
...
let test_content_digest_verify_mismatch () =
  ...
  let digest = Signature.Content_digest.compute ~algorithm:`Sha256 ~body in
  match
    Signature.Content_digest.verify ~header:digest ~body:"Different content"
  with
...
let test_hmac_sign_verify clock =
  ...
  let config =
    Signature.config ~key ~keyid:"test-key-1"
  ...
  | Error e ->
      Alcotest.fail ("Signing failed: " ^ Signature.sign_error_to_string e)
  | Ok signed_headers -> (
      Alcotest.(check bool)
        "Has Signature header" true
        (Option.is_some (get "signature" signed_headers));
      Alcotest.(check bool)
        "Has Signature-Input header" true
        (Option.is_some (get "signature-input" signed_headers));
      match
        Signature.verify ~clock ~key ~context ~headers:signed_headers ()
      with
      | Error e ->
          Alcotest.fail
            ("Verification failed: " ^ Signature.verify_error_to_string e)
      | Ok result ->
          Alcotest.(check (option string))
            "Keyid matches" (Some "test-key-1") result.keyid)
...
let test_ed25519_sign_verify clock =
  ...
  let config =
    Signature.config ~key ~keyid:"ed25519-key"
      ~components:
        [ Signature.Component.method_; Signature.Component.path;
  ...
  | Error e ->
      Alcotest.fail
        ("Ed25519 signing failed: " ^ Signature.sign_error_to_string e)
  | Ok signed_headers -> (
      match Signature.verify ~clock ~key ~context ~headers:signed_headers () with
      | Error e ->
          Alcotest.fail
            ("Ed25519 verification failed: "
           ^ Signature.verify_error_to_string e)
      | Ok result ->
          Alcotest.(check (option string))
            "Keyid matches" (Some "ed25519-key") result.keyid;
          Alcotest.(check int)
            "Verified 3 components" 3
            (List.length result.verified_components))
...
let test_signature_base_format clock =
  ...
  let config =
    Signature.config ~key ~keyid:"test"
      ~components:[ Signature.Component.method_; Signature.Component.authority ]
  ...
  | Error e ->
      Alcotest.fail ("Signing failed: " ^ Signature.sign_error_to_string e)
  | Ok signed_headers -> (
      match get "signature-input" signed_headers with
...
let test_sign_with_digest_covers_digest clock =
  ...
  let config =
    Signature.config ~key ~keyid:"digest-key"
      ~components:
        [ Signature.Component.method_; Signature.Component.authority;
  ...
  let body = "{\"hello\": \"world\"}" in
  match
    Signature.sign_with_digest ~clock ~config ~context ~headers ~body
      ~digest_algorithm:`Sha256
  with
  | Error e ->
      Alcotest.fail
        ("sign_with_digest failed: " ^ Signature.sign_error_to_string e)
  | Ok signed -> (
      Alcotest.(check (option string))
        "Content-Digest added"
        (Some (Signature.Content_digest.compute ~algorithm:`Sha256 ~body))
        (get "content-digest" signed);
      (* The verifier sees the full header block, digest included. *)
      let context = Signature.Context.request ~method_:`POST ~uri ~headers:signed in
      match Signature.verify ~clock ~key ~context ~headers:signed () with
      | Error e ->
          Alcotest.fail
            ("Verification failed: " ^ Signature.verify_error_to_string e)
      | Ok result ->
          Alcotest.(check int)
            "content-digest is covered" 4
            (List.length result.verified_components))

(** {1 Middleware Tests} *)

let test_middleware_signs_string_body () =
  Eio_main.run @@ fun env ->
  Eio.Switch.run @@ fun sw ->
  let clock = Eio.Stdenv.clock env in
  let key = Signature.Key.symmetric "middleware-secret" in
  ...
  let t = Signature.Middleware.sign ~clock ~config ~key backend in
  let _ =
    Fetch.post ~sw t "https://example.com/inbox"
      ~body:(Fetch.String "{\"hello\": \"world\"}")
  in
  match !seen with
  | None -> Alcotest.fail "backend never saw a request"
  | Some req ->
      Alcotest.(check bool)
        "Signature added" true
        (Option.is_some (get "signature" req.headers));
      Alcotest.(check bool)
        "Date added" true
        (Option.is_some (get "date" req.headers));
      Alcotest.(check bool)
        "Content-Digest added" true
      ...
      in
      (match
         Signature.verify ~clock ~key ~context ~headers:req.headers ()
       with
      | Ok result ->
          Alcotest.(check (option string))
            "keyid" (Some "mw-key") result.keyid
      | Error e ->
          Alcotest.fail
            ("Middleware signature did not verify: "
           ^ Signature.verify_error_to_string e))

let test_middleware_rejects_stream_body () =
  Eio_main.run @@ fun env ->
  Eio.Switch.run @@ fun sw ->
  let clock = Eio.Stdenv.clock env in
  let key = Signature.Key.symmetric "middleware-secret" in
  ...
  let body = Fetch.stream (Eio.Flow.string_source "streamed") in
  match Fetch.post ~sw t "https://example.com/inbox" ~body with
  | _ -> Alcotest.fail "a streaming body should not be signable"
  | exception Eio.Io (Fetch.E (Fetch.Invalid_request _), _) -> ()
...
let algorithm_tests = [ ("Algorithm roundtrip", `Quick, test_algorithm_roundtrip) ]
...
let middleware_tests =
  [
    ("String body is signed", `Quick, test_middleware_signs_string_body);
    ("Stream body is refused", `Quick, test_middleware_rejects_stream_body);
  ]
