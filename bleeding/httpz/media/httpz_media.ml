...
let sanitize_diagnostic = Httpz_diagnostic.sanitize
...
module Syntax = struct
  ...
  let[@zero_alloc] skip_space (local_ s : string) first stop =
    Httpz_syntax.skip_space s first stop
  ;;

  let[@zero_alloc] trim_space (local_ s : string) first stop =
    Httpz_syntax.trim_space s first stop
  ;;
  ...
  let rec field_value (local_ s : string) i stop =
    i = stop || (Httpz_syntax.is_field_value_char (char_at s i) && field_value s (i + 1) stop)
  ;;

  let rec token (local_ s : string) i stop =
    i = stop || (Httpz_syntax.is_token_char (char_at s i) && token s (i + 1) stop)
  ;;
  ...
  let[@inline][@zero_alloc] equal_ci
    ...
    Httpz_syntax.equal_ci a a0 b b0 len
  ;;
...
let is_token = Httpz_syntax.is_token
...
let check_type ~wildcards ~what s =
  ...
  if valid s ~pos:0 ~len:(String.length s)
     && not (Httpz_syntax.is_space (Char_u.of_char s.[0]))
     && not (Httpz_syntax.is_space (Char_u.of_char s.[String.length s - 1]))
...
let parameter_value value =
  if is_token value then value else Httpz_syntax.quote_string value
...
let[@zero_alloc] matches ~(range : string @ local) (media : string @ local) =
  ...
  Httpz_syntax.is_field_value range
