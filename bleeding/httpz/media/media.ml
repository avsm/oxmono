...
let sanitize_diagnostic = Httpz.Diagnostic.sanitize
...
(* The unboxed bounds keep hot matching paths allocation-free. *)
module Syntax = struct
  ...
  let[@zero_alloc] skip_space (local_ s : string) first stop =
    Httpz.Syntax.skip_space s first stop
  ;;

  let[@zero_alloc] trim_space (local_ s : string) first stop =
    Httpz.Syntax.trim_space s first stop
  ;;
  ...
  let rec field_value (local_ s : string) i stop =
    i = stop || (Httpz.Syntax.is_field_value_char (char_at s i) && field_value s (i + 1) stop)
  ;;

  let rec token (local_ s : string) i stop =
    i = stop || (Httpz.Syntax.is_token_char (char_at s i) && token s (i + 1) stop)
  ;;

  let[@inline] parts (local_ s : string) pos len =
    let first = skip_space s pos (pos + len) in
    let last = trim_space s first (pos + len) in
  ...
  let[@inline][@zero_alloc] equal_ci
      (local_ a : string) a0 (local_ b : string) b0 len =
    Httpz.Syntax.equal_ci a a0 b b0 len
  ;;

  let[@inline] equal_part
      (local_ a : string) a0 a1 (local_ b : string) b0 b1 =
    a1 - a0 = b1 - b0 && equal_ci a a0 b b0 (a1 - a0)
  ;;
...
let is_token = Httpz.Syntax.is_token
...
let check_type ~wildcards ~what s =
  ...
  if valid s ~pos:0 ~len:(String.length s)
     && not (Httpz.Syntax.is_space (Char_u.of_char s.[0]))
     && not (Httpz.Syntax.is_space (Char_u.of_char s.[String.length s - 1]))
...
let check_names ~accept ~params media =
...
let make ?(accept = []) ?(params = []) media ~encode ~decode ~encode_string
    ~decode_string =
  let media, accept = check_names ~accept ~params media in
  { media
  ; params
  ; accept
  ; encode
...
let parameter_value value =
  if is_token value then value else Httpz.Syntax.quote_string value
...
let[@zero_alloc] matches ~(range : string @ local) (media : string @ local) =
  ...
  Httpz.Syntax.is_field_value range
...
let accept_header medias =
  ...
  let medias = List.map (check_type ~wildcards:true ~what:"preference") medias in
...
let lines ?(accept = []) ?(params = []) media item =
  let smedia, saccept = check_names ~accept ~params media in
  { smedia; sparams = params; saccept; item }
;;
...
let add_item b s x =
  Buffer.add_string b (encode s.item x);
  Buffer.add_char b '\n'
;;
...
let decode_items s body =
  ...
  let rec go acc first =
    if first >= len then Ok (List.rev acc)
    else
      let last = match String.index_from_opt body first '\n' with None -> len | Some i -> i in
      (* A line holding only the carriage return of a CRLF is the blank line, since the
         stripping below removes that byte. *)
      let is_blank = last = first || (last = first + 1 && Char.equal body.[first] '\r') in
      if is_blank then go acc (last + 1)
      else
        let stop = if last > first && body.[last - 1] = '\r' then last - 1 else last in
        match decode s.item (String.sub body first (stop - first)) with
        | Ok x -> go (x :: acc) (last + 1)
        | Error _ as e -> e
  in
  go [] 0
;;
