(** This module provides typed media codecs.
    ...
    {[
    let integer : int Httpz_media.t =
      Httpz_media.of_strings "text/plain"
    ...
    JSON, JSON Lines, Markdown and HTML codecs are included in this library.
    [fetch] and [proffer] expose them as [Json] and [Markdown]. *)
...
module Syntax : sig
  ...
  (** [skip_space s first stop] is the offset of the first byte of [s] in
      \[[first], [stop]) that is neither SP nor HTAB, or [stop] when every byte
      of the slice is one. The bounds are not checked. *)

  val[@zero_alloc] trim_space : local_ string -> int -> int -> int @@ portable
  (** [trim_space s first stop] is one past the last byte of [s] in
      \[[first], [stop]) that is neither SP nor HTAB, or [first] when every
      byte of the slice is one. The bounds are not checked. *)

  val[@zero_alloc] equal_ci :
    local_ string -> int -> local_ string -> int -> int -> bool @@ portable
  (** [equal_ci a a_pos b b_pos len] is whether the [len] bytes of [a] from
      [a_pos] and of [b] from [b_pos] are equal with ASCII case folded. The
...
(** [accept_header medias] is an Accept field value listing [medias] in order
    ...
    places. Each entry is validated as {!v} validates an [accept] entry, so a
    media type or range may be given but nothing else can reach the field
    value. Case is folded. Up to 1000 entries are supported.
    @raise Stdlib.Invalid_argument if more than 1000 entries are supplied, or
    if an entry is not a media type or range. *)
...
(** [lines media item] is a sequence codec for [media] in which each line
    ...
    A trailing carriage return is removed from each line, and a line that is
    then empty is skipped when decoding. A line of blanks is not empty and is
    decoded like any other. [accept] and [params] are as in {!v}. *)
val lines
  :  ?accept:string list
  -> ?params:(string * string) list
  -> string
