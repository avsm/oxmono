...
type part = {
  ...
  filename : string option;
      (** [filename] is the [filename*] parameter when present and otherwise the
          [filename] parameter, or [None] when the part is not a file. A
          [filename*] that is present but is not a UTF-8 RFC 8187 [ext-value]
          rejects the part rather than falling back, since the two parameters
          need not name the same file. It is untrusted display metadata, not a
          safe filesystem path: it can contain separators, dot segments, or NUL
...
val content_local : string @ local -> part -> string @ local @@ portable
val blit_content : string @ local -> part -> bytes @ local -> off:int -> unit @@ portable
(** Copy a part into the caller region or a supplied destination. These avoid
    an owned intermediate string; retained content still needs heap lifetime. *)
...
(** [boundary_of_content_type ct] is the [boundary] parameter of [ct] when [ct]
    ...
    another exact base type and defaults to [multipart/form-data]; it may be
    given in any case. The media type is
...
(** [parse ~boundary body] is the parts of [body], in order. [max_parts] bounds
    ...
    delimiter are ignored. RFC 2046 counts the CRLF after the closing delimiter
    as the first bytes of that epilogue, so a body ending at the delimiter is
    complete. A part must carry a Content-Disposition field of
    ...
    part missing its Content-Disposition, an undecodable [filename*], a body
    truncated before the closing delimiter, or more than [max_parts] parts. *)
