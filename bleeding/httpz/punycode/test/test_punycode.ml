...
(* RFC 1034 caps a DNS label at 63 octets, and RFC 5890 2.3.2.1 carries the cap
   to the A-label. The cap also bounds Punycode decoding, which is quadratic in
   the payload length. *)
let test_idna_label_length_cap () =
  let long = String.make 64 'a' in
  let at_cap = String.make 63 'a' in
  let raises what f =
    match f () with
    | (_ : string) -> fail (what ^ ": over-long label accepted")
    | exception Punycode_idna.Error (Punycode_idna.Punycode_error
        (Punycode.Label_too_long _)) -> ()
    | exception Punycode_idna.Error e ->
        fail (Format.asprintf "%s: %a" what Punycode_idna.pp_error_reason e)
  in
  raises "to_ascii" (fun () -> Punycode_idna.to_ascii (long ^ ".example.com"));
  raises "to_unicode" (fun () -> Punycode_idna.to_unicode (long ^ ".example.com"));
  raises "to_unicode ace" (fun () ->
      Punycode_idna.to_unicode ("xn--" ^ String.make 60 'a' ^ ".example.com"));
  check string "to_ascii at cap" (at_cap ^ ".example.com")
    (Punycode_idna.to_ascii (at_cap ^ ".example.com"));
  check string "to_unicode at cap" (at_cap ^ ".example.com")
    (Punycode_idna.to_unicode (at_cap ^ ".example.com"))

let test_idna_hyphens_and_alabels () =
  let rejects what value =
    match Punycode_idna.label_to_ascii value with
    | (_ : string) -> fail (what ^ ": invalid label accepted")
    | exception Punycode_idna.Error _ -> ()
  in
  List.iter (fun label -> rejects label label) [ "-abc"; "abc-"; "ab--cd" ];
  rejects "empty ACE payload" "xn--";
  rejects "ACE payload for ASCII only" "xn--abc-";
  check string "valid A-label retained" "xn--mnchen-3ya"
    (Punycode_idna.label_to_ascii "xn--mnchen-3ya");
  let too_long = "xn--" ^ String.make 60 'a' in
  (match Punycode.decode_label too_long with
   | (_ : string) -> fail "decode_label accepted 64-byte input"
   | exception Punycode.Error (Punycode.Label_too_long 64) -> ()
   | exception Punycode.Error e ->
      fail (Format.asprintf "wrong decode_label error: %a" Punycode.pp_error_reason e))

let test_idna_utf8_root_and_ulabel_length () =
  let rejects_invalid label convert value =
    match convert value with
    | (_ : string) -> fail (label ^ " was accepted")
    | exception Punycode_idna.Error (Punycode_idna.Invalid_label _) -> ()
    | exception Punycode_idna.Error error ->
        fail (Format.asprintf "wrong %s error: %a" label
          Punycode_idna.pp_error_reason error)
  in
  rejects_invalid "domain malformed UTF-8" Punycode_idna.to_ascii
    "bad\255.example";
  rejects_invalid "label malformed UTF-8" Punycode_idna.label_to_ascii "bad\255";
  rejects_invalid "display malformed UTF-8" Punycode_idna.to_unicode "bad\255";
  check string "literal replacement character remains valid" "xn--zn7c"
    (Punycode_idna.to_ascii "\xef\xbf\xbd");
  check string "ASCII root dot" "example.com."
    (Punycode_idna.to_ascii "example.com.");
  check string "Unicode root dot" "xn--mnchen-3ya.example."
    (Punycode_idna.to_ascii "münchen.example.");
  check string "Unicode root dot decode" "münchen.example."
    (Punycode_idna.to_unicode "xn--mnchen-3ya.example.");
  let long_ulabel = String.concat "" (List.init 40 (fun _ -> "é")) in
  let alabel = Punycode_idna.label_to_ascii long_ulabel in
  check bool "long UTF-8 U-label checked after encoding" true
    (String.length long_ulabel > 63 && String.length alabel <= 63);
  let excessive_ulabel = String.concat "" (List.init 513 (fun _ -> "é")) in
  rejects_invalid "oversized U-label encode" Punycode_idna.label_to_ascii
    excessive_ulabel;
...
let idna_tests =
  [
    ...
    ("label length cap", `Quick, test_idna_label_length_cap);
    ("hyphens and A-labels", `Quick, test_idna_hyphens_and_alabels);
