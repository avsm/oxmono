...
(* A part wider than the whole address is rejected as it is scanned, so the
...
let[@zero_alloc] ipv4_of_string_or_null s =
  ...
  then Null
    ...
    then Null
      ...
      if !bad then Null else This !acc))
;;

let ipv4_of_string s =
  match ipv4_of_string_or_null s with Null -> None | This v -> Some v
;;

let ipv4_canonical s =
  match ipv4_of_string_or_null s with
  | Null -> None
  | This v ->
...
let[@zero_alloc] is_ipv4_literal s =
  match ipv4_of_string_or_null s with
  | Null -> false
  | This _ -> true
;;
...
let is_ipv6_literal s =
  ...
  | None -> Uri.Scanner.is_ipv6 value
  | Some zone ->
    ...
    && Uri.Scanner.is_ipv6 (String.sub value ~pos:0 ~len:zone)
;;
