...
let login_action ~http_config ~server ~api_key ~email ~password ~profile ~key_name env =
  ...
  | None, None ->
      ...
      Eio.Switch.run @@ fun sw ->
      let client = Client.login_api_key ~sw ~env ~http_config ?profile ~server_url:server ~api_key ?key_name () in
  ...
  | None, Some email ->
      ...
      in
      Eio.Switch.run @@ fun sw ->
      let client = Client.login_password ~sw ~env ~http_config ?profile ~server_url:server ~email ~password () in
...
let login_cmd env fs =
  ...
  let login' (style_renderer, level) http_config server api_key email password profile key_name =
    setup_logging_with_config style_renderer level http_config;
    Error.wrap (fun () ->
      login_action ~http_config ~server ~api_key ~email ~password ~profile ~key_name env)
  in
  Cmd.v info
    Term.(const login' $ setup_logging $ http_config_term fs $ server_arg $ api_key_arg $ email_arg $ password_arg $ profile_arg $ key_name_arg)
