...
presence with `Option.join`. The full workspace check also updated Bushel's
Karakeep summary and typed PeerTube ID/UUID/nullable-field adapters. All three
CLI `--help=plain` startup checks pass; the subsequent workspace `@all` and
Bushel tests pass under `release-check`.
