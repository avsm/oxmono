...
type t = { client : Fetch.plain; root_url : Url.t; limits : Davz.limits }
exception Protocol_error of string
type http_error = { status : int; headers : Http.Header.t; body : string;
                    truncated : bool; dav_errors : Davz.element list }
exception Http_error of http_error
let protocol = function Ok v -> v | Error s -> raise (Protocol_error s)
let url_exn = function Ok v -> v | Error s -> invalid_arg ("Proffer_dav: " ^ s)
let root t = Url.to_string t.root_url
let v ?(limits = Davz.default_limits) ~root client =
  Davz.validate_limits limits;
  let root_url = url_exn (Url.of_string root) in
  let u = Uriz.of_string_exn root in
  if Uriz.query u <> Null || Uriz.fragment u <> Null ||
    not (String.ends_with ~suffix:"/" (Uriz.path u)) then
    invalid_arg "Proffer_dav: root must be a collection URL without query or fragment";
  { client = Fetch.restrict ~under:[Url.to_string root_url] client; root_url; limits }
let resolve t reference =
  let u = Uriz.of_string_exn reference in
  if Uriz.fragment u <> Null then invalid_arg "Proffer_dav: fragments are not resource paths";
  let url = url_exn (Url.resolve ~base:t.root_url reference) in
  if not (Url.under ~prefix:t.root_url url) then invalid_arg "Proffer_dav: URL outside root";
  Url.to_string url
let child t ~collection name =
  if name = "" || name = "." || name = ".." || String.contains name '/' || String.contains name '\\' then
    invalid_arg "Proffer_dav.child: invalid segment";
  let collection = resolve t collection in
  let u = Uriz.of_string_exn collection in
  if Uriz.query u <> Null || not (String.ends_with ~suffix:"/" (Uriz.path u)) then
    invalid_arg "Proffer_dav.child: collection must end in / and have no query";
  resolve t (collection ^ Uriz.pct_encode ~component:`Unreserved name)

module Header = struct
  let depth = Fetch.Header.v "Depth" ~encode:Davz.encode_depth ~decode:Davz.decode_depth
  ...
  let lock_token = Fetch.Header.v "Lock-Token" ~encode:Davz.Token.encode ~decode:Davz.Token.decode
  let split_dav s =
...
let xml t response =
  ...
  protocol (Davz.parse_xml ~limits:t.limits ?encoding body)
let multi t response = protocol (Davz.multistatus (xml t response))
let reject t response =
  let body, truncated = read_bounded t.limits.max_bytes response in
  let dav_errors =
    if truncated then [] else
    try match Davz.parse_xml ~limits:t.limits ?encoding:(encoding response body) body with
      | Ok e when e.name = Davz.dav "error" -> List.filter_map (function
          | Davz.Element e -> Some e | _ -> None) e.children
      | _ -> []
...
let propfind ?(depth = `Zero) t target query =
  ...
  request ~headers ~body:(Fetch.String (Davz.propfind query)) t "PROPFIND" target (fun r ->
...
let conditions ?(condition = Unconditional) ?if_ () =
  let hs = match condition with
    ...
    | If_match validator ->
        if validator.weak then invalid_arg "Proffer_dav: If-Match requires a strong ETag";
        Fetch.Header.[if_match, `Etags [validator]]
    | If_absent -> Fetch.Header.[if_none_match, `Any] in
  match if_ with None -> hs | Some value ->
    Fetch.Header.append hs Fetch.Header.[raw "If" (Davz.encode_if value)]
...
let proppatch ?condition ?if_ t target updates =
  ...
  request ~headers ~body:(Fetch.String (Davz.proppatch updates)) t "PROPPATCH" target (fun r ->
    if Fetch.status r <> 207 then reject t r;
    multi t r)
...
let copy ?(depth = `Infinity) ?overwrite ?if_ t ~src ~dst () =
  transfer ?overwrite ?if_ ~depth:(depth : Davz.tree_depth :> Davz.depth) t "COPY" ~src ~dst ()
...
let lock_condition lease = Davz.Tagged [lease.url, [[Davz.Is (Davz.Token lease.token)]]]
let timeout_headers timeout = Fetch.Header.[raw "Timeout" (Davz.encode_timeout timeout)]
let granted t r token =
  let locks = protocol (Davz.locks (xml t r)) in
  match List.filter (fun (l : Davz.lock) -> l.token = Some token) locks with
  | [granted] -> { url = Fetch.url r; token; granted }
  | _ -> raise (Protocol_error "lock response must contain the granted token exactly once")
let lock ?(scope = Davz.Exclusive) ?(depth = `Zero) ?(timeout = Davz.Seconds 600L) ?owner t target =
  let headers = Fetch.Header.append xml_headers
    (Fetch.Header.append Fetch.Header.[Header.depth, (depth : Davz.tree_depth :> Davz.depth)] (timeout_headers timeout)) in
  request ~headers ~body:(Fetch.String (Davz.lockinfo ?owner scope)) t "LOCK" target (fun r ->
...
let refresh_lock ?(timeout = Davz.Seconds 600L) t lease =
  let if_ = Davz.Untagged [[Davz.Is (Davz.Token lease.token)]] in
