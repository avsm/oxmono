...
let test_roundtrip input =
  match Punycode.encode_utf8 input with
  | Ok encoded -> (
      ...
      | Error _ ->
          (* Some encoded values might not decode, that's ok for fuzz testing *)
          check true)
  | Error _ ->
      (* Some inputs might not encode, that's ok *)
      check true
...
let test_ascii_string input =
  if String.length input > 0 then begin
    ...
    if String.length ascii_only > 0 then
      ignore (Punycode.encode_utf8 ascii_only)
  end;
  check true
...
let test_ace_prefix input =
  ...
  ignore (Punycode.decode_utf8 ace_input);
  check true
