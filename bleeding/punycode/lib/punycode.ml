...
let overflow pos = Error (Overflow pos)
let invalid_character pos u = Error (Invalid_character (pos, u))
let invalid_digit pos c = Error (Invalid_digit (pos, c))
let unexpected_end pos = Error (Unexpected_end pos)
let _invalid_utf8 pos = Error (Invalid_utf8 pos)
let label_too_long len = Error (Label_too_long len)
let empty_label = Error Empty_label
...
let utf8_to_codepoints s =
  ...
  while !byte_offset < len && !error = None do
...
let encode_impl codepoints case_flags =
  ...
  if input_length = 0 then Ok ""
    ...
    while !h < input_length && !result = Ok () do
      ...
      match safe_mul_add !delta (m - !n) (!h + 1) pos with
      ...
      | Ok new_delta ->
          ...
          let j = ref 0 in
          while !j < input_length && !result = Ok () do
  ...
  if input_length = 0 then Ok ([||], [||])
    ...
    for j = 0 to b - 1 do
      if !error = None then begin
        ...
        let code = Char.code c in
        if code >= 0x80 then
          error := Some (Invalid_character (pos, Uchar.of_int code))
        else begin
    ...
    | None -> (
        ...
        while !in_pos < input_length && !result = Ok () do
          let oldi = !i in
          let w = ref 1 in
          let k = ref base in
          let done_decoding = ref false in

          while (not !done_decoding) && !result = Ok () do
            let pos =
              { byte_offset = !in_pos; char_index = Array.length !output }
            in

            if !in_pos >= input_length then begin
              result := unexpected_end pos;
              done_decoding := true
            end
            else begin
              ...
              | None ->
                  result := invalid_digit pos c;
                  done_decoding := true
              | Some digit -> (
                  (* i = i + digit * w, with overflow check *)
                  match safe_mul_add !i digit !w pos with
                  | Error e ->
                      result := Error e;
                      done_decoding := true
                  | Ok new_i ->
                      ...
                      if digit < t then begin
                        ...
                        if !w > max_int_value / base_minus_t then begin
                          result := overflow pos;
                          done_decoding := true
                        end
                        else begin
          ...
          if !result = Ok () then begin
            ...
            if increment > max_int_value - !n then result := overflow pos
            else begin
              n := !n + increment;
              i := !i mod (out_len + 1);
              ...
              if not (Uchar.is_valid !n) then
                result := invalid_character pos Uchar.rep
              else begin
        ...
        match !result with
        | Error e -> Error e
        | Ok () -> Ok (!output, !case_output))
  end

let decode input = Result.map fst (decode_impl input)
...
let encode_label label =
  if String.length label = 0 then empty_label
    ...
    if len > max_label_length then label_too_long len else Ok label
  end
  else
    ...
    if len > max_label_length then label_too_long len else Ok result
...
let decode_label label =
  if String.length label = 0 then empty_label
    ...
    if is_ascii_string label then Ok label
    else
