...
let validate_node_name name =
  if name = "" then Error "node name must not be empty"
  else if String.contains name '/' then Error "node name must not contain '/'"
  else if name = "." || name = ".." then Error "node name must not be '.' or '..'"
  else if String.length name >= 2 && name.[0] = '_' && name.[1] = '_' then
    Error "node name must not start with '__'"
  else Ok ()
...
let array_of_json json_str =
  ...
  try
    ...
    let* data_type = match Zarr_dtype.of_string dtype_str with
      | Some d -> Ok d | None -> Error ("unsupported dtype: " ^ dtype_str) in
    ...
    let* codecs = match Zarr_codec.specs_of_json (Json_util.(member "codecs" json |> to_list_exn)) with
      | Ok c -> Ok c | Error (`Codec_error m) -> Error m | Error _ -> Error "codec parse error" in
    let has_a2b = List.exists (function
      | Zarr_codec.Bytes _ | Sharding _ | Extension _ -> true | _ -> false
    ) codecs in
    let* () =
      if not has_a2b then Error "codecs must contain an array->bytes codec"
      else Ok () in
    ...
    Ok { zarr_format; shape; data_type; chunk_grid; chunk_key_encoding;
         fill_value; codecs; dimension_names; attributes }
  with
  | Failure msg -> Error ("JSON error: " ^ msg)
...
let group_of_json json_str =
  try
    ...
    if zf <> 3 then Error ("zarr_format must be 3, got " ^ string_of_int zf)
    else
      let nt = Json_util.(member "node_type" json |> to_string_exn) in
      if nt <> "group" then Error "node_type must be 'group'"
      else
        let attr_json = Json_util.member "attributes" json in
        let attributes = if Json_util.is_null attr_json then None else Some attr_json in
        Ok { zarr_format = zf; attributes }
  with
  | Failure msg -> Error ("JSON error: " ^ msg)
