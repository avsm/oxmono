...
let test_parse_spec_example () =
  let meta = Metadata.array_of_json spec_example_array in
...
let test_reject_zarr_format_2 () =
  let json = {| {"zarr_format": 2, "node_type": "array", "shape": [10], "data_type": "int32",
                 ...
                 "fill_value": 0} |} in
  (try
    ignore (Metadata.array_of_json json);
    fail "should reject zarr_format 2"
  with Failure msg ->
...
let test_reject_node_type_group () =
  ...
  (try
    ignore (Metadata.array_of_json json);
    fail "should reject node_type group for array"
  with Failure _ -> ())
...
let test_reject_missing_array_to_bytes () =
  ...
  (try
    ignore (Metadata.array_of_json json);
    fail "should reject missing array->bytes codec"
  with Failure msg | Codec.Codec_error msg ->
...
let test_parse_group_metadata () =
  ...
  let meta = Metadata.group_of_json json in
  check int "zarr_format" 3 meta.zarr_format;
...
let test_group_roundtrip () =
  ...
  let meta2 = Metadata.group_of_json json in
  check int "zarr_format" meta.zarr_format meta2.zarr_format
