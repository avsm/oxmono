...
(** Array fill values.
    ...
    array's data type. The JSON lexicon below is the
    {{:https://zarr-specs.readthedocs.io/en/latest/v3/core/}specification's},
    matched against [zarrs_metadata::v3::array::fill_value]. *)
...
(** [of_json dt j] decodes the [fill_value] member [j] of an array whose
    ...
    {- integers: a JSON number with no fractional part, within the
       ...
       integer. That is a departure from the oracle, which parses the
       full 64 bit ranges.}
    {- floats: a JSON number, or one of the strings ["Infinity"],
       ["-Infinity"], ["NaN"], or a hex string of the big endian byte
       image, whose byte count must equal the type's size. The [0x]
       prefix and the digits are both accepted in either case, where
       the oracle requires a lowercase prefix. ["NaN"] decodes to the
    ...
    {- [Complex64] and [Complex128]: a two element JSON array of the
       ...
       the component type.}
    {- [Raw n]: a JSON array of [n] numbers in \[0;255\], or a base64
...
(** [to_json dt t] is the [fill_value] member for [t] under data type
    [dt]. It reverses {!of_json}: an infinity becomes ["Infinity"] or
    ["-Infinity"], the canonical NaN becomes ["NaN"], any other NaN
    becomes the lowercase hex string of its big endian bytes, a finite
    float becomes a JSON number, and [Raw n] becomes an array of byte
    numbers, never base64.
