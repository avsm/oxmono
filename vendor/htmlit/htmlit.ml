...
module El = struct
  ...
  (* Upstream tests this membership against a module-level [Set.Make (String)].
     A set does not cross portability, so a module-level one cannot be read
     from the portable [add_child]. Matching on the name needs no module-level
     value at all, and the same fourteen names answer true. *)
