...
module type Parameters = sig @@ portable
  val a : field_element
  val b : field_element
  ...
  val n : field_element
  val pident: string
  val byte_length : int @@ portable
  val bit_length : int @@ portable
  val fe_length : int
  val first_byte_bits : int option @@ portable
end
...
module type Foreign = sig @@ portable
...
module type Field_element = sig @@ portable
...
module type Point = sig @@ portable
  val at_infinity : unit -> point
  val is_infinity : point -> bool
  val add : point -> point -> point @@ portable
...
module type Scalar = sig @@ portable
  val not_zero : string -> bool
  val is_in_range : string -> bool @@ portable
  val of_octets : string -> (scalar, error) result @@ portable
  val to_octets : scalar -> string
  val scalar_mult : scalar -> point -> point @@ portable
  val scalar_mult_base : scalar -> point @@ portable
...
module Make_dh (Param : Parameters) (P : Point) (S : Scalar) : Dh = struct
  ...
  let (gen_key_with @ portable) ~g ?compress () =
    let rec generate () =
      let candidate =
        Mirage_crypto_rng.generate_portable ~g Param.byte_length
      in
      match S.of_octets candidate with
      | Ok secret -> secret
      | Error _ -> generate ()
    in
    let private_key = generate () in
    private_key, share ?compress private_key

  let (key_exchange @ portable) secret received =
    match point_of_octets received with
    | Error _ as err -> err
    | Ok shared -> Ok (P.x_of_finite_point (S.scalar_mult secret shared))
end

module type Foreign_n = sig @@ portable
  val mul : out_field_element -> field_element -> field_element -> unit
  val add : out_field_element -> field_element -> field_element -> unit
  val inv : out_field_element -> field_element -> unit
...
module type Fn = sig @@ portable
  val from_be_octets : string -> field_element @@ portable
  val to_be_octets : field_element -> string @@ portable
  val mul : field_element -> field_element -> field_element @@ portable
  val add : field_element -> field_element -> field_element
  val inv : field_element -> field_element @@ portable
  val one : field_element @@ portable
  val from_montgomery : field_element -> field_element @@ portable
  val to_montgomery : field_element -> field_element @@ portable
end
...
module Make_dsa (Param : Parameters) (F : Fn) (P : Point) (S : Scalar) (H : Digestif.S) = struct
  ...
  let priv_of_octets : _ @ portable = fun buf -> S.of_octets buf
  ...
  let pub_of_octets : _ @ portable = fun buf ->
  ...
  let verify : _ @ portable = fun ~key (r, s) msg ->
    try
...
module P256 : Dh_dsa  = struct
  ...
  module Foreign = struct
    external mul : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_p256_mul" [@@noalloc]
    external sub : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_p256_sub" [@@noalloc]
    external add : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_p256_add" [@@noalloc]
    external to_montgomery : out_field_element -> field_element -> unit @@ portable = "mc_p256_to_montgomery" [@@noalloc]
    external from_octets : out_field_element -> string -> unit @@ portable = "mc_p256_from_bytes" [@@noalloc]
    external set_one : out_field_element -> unit @@ portable = "mc_p256_set_one" [@@noalloc]
    external nz : field_element -> bool @@ portable = "mc_p256_nz" [@@noalloc]
    external sqr : out_field_element -> field_element -> unit @@ portable = "mc_p256_sqr" [@@noalloc]
    external from_montgomery : out_field_element -> field_element -> unit @@ portable = "mc_p256_from_montgomery" [@@noalloc]
    external to_octets : bytes -> field_element -> unit @@ portable = "mc_p256_to_bytes" [@@noalloc]
    external inv : out_field_element -> field_element -> unit @@ portable = "mc_p256_inv" [@@noalloc]
    external select_c : out_field_element -> bool -> field_element -> field_element -> unit @@ portable = "mc_p256_select" [@@noalloc]
    external double_c : out_point -> point -> unit @@ portable = "mc_p256_point_double" [@@noalloc]
    external add_c : out_point -> point -> point -> unit @@ portable = "mc_p256_point_add" [@@noalloc]
    external scalar_mult_base_c : out_point -> string -> unit @@ portable = "mc_p256_scalar_mult_base" [@@noalloc]
  end

  module Foreign_n = struct
    external mul : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_np256_mul" [@@noalloc]
    external add : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_np256_add" [@@noalloc]
    external inv : out_field_element -> field_element -> unit @@ portable = "mc_np256_inv" [@@noalloc]
    external one : out_field_element -> unit @@ portable = "mc_np256_one" [@@noalloc]
    external from_bytes : out_field_element -> string -> unit @@ portable = "mc_np256_from_bytes" [@@noalloc]
    external to_bytes : bytes -> field_element -> unit @@ portable = "mc_np256_to_bytes" [@@noalloc]
    external from_montgomery : out_field_element -> field_element -> unit @@ portable = "mc_np256_from_montgomery" [@@noalloc]
    external to_montgomery : out_field_element -> field_element -> unit @@ portable = "mc_np256_to_montgomery" [@@noalloc]
  end
...
"\x36\x17\xde\x4a\x96\x26\x2c\x6f\x5d\x9e\x98\xbf\x92\x92\xdc\x29\xf8\xf4\x1d\xbd\x28\x9a\x14\x7c\xe9\xda\x31\x13\xb5\xf0\xb8\xc0\x0a\x60\xb1\xce\x1d\x7e\x81\x9d\x7a\x43\x1d\x7c\x90\xea\x0e\x5f"
  ...
  module Foreign = struct
    external mul : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_p384_mul" [@@noalloc]
    external sub : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_p384_sub" [@@noalloc]
    external add : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_p384_add" [@@noalloc]
    external to_montgomery : out_field_element -> field_element -> unit @@ portable = "mc_p384_to_montgomery" [@@noalloc]
    external from_octets : out_field_element -> string -> unit @@ portable = "mc_p384_from_bytes" [@@noalloc]
    external set_one : out_field_element -> unit @@ portable = "mc_p384_set_one" [@@noalloc]
    external nz : field_element -> bool @@ portable = "mc_p384_nz" [@@noalloc]
    external sqr : out_field_element -> field_element -> unit @@ portable = "mc_p384_sqr" [@@noalloc]
    external from_montgomery : out_field_element -> field_element -> unit @@ portable = "mc_p384_from_montgomery" [@@noalloc]
    external to_octets : bytes -> field_element -> unit @@ portable = "mc_p384_to_bytes" [@@noalloc]
    external inv : out_field_element -> field_element -> unit @@ portable = "mc_p384_inv" [@@noalloc]
    external select_c : out_field_element -> bool -> field_element -> field_element -> unit @@ portable = "mc_p384_select" [@@noalloc]
    external double_c : out_point -> point -> unit @@ portable = "mc_p384_point_double" [@@noalloc]
    external add_c : out_point -> point -> point -> unit @@ portable = "mc_p384_point_add" [@@noalloc]
    external scalar_mult_base_c : out_point -> string -> unit @@ portable = "mc_p384_scalar_mult_base" [@@noalloc]
  end

  module Foreign_n = struct
    external mul : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_np384_mul" [@@noalloc]
    external add : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_np384_add" [@@noalloc]
    external inv : out_field_element -> field_element -> unit @@ portable = "mc_np384_inv" [@@noalloc]
    external one : out_field_element -> unit @@ portable = "mc_np384_one" [@@noalloc]
    external from_bytes : out_field_element -> string -> unit @@ portable = "mc_np384_from_bytes" [@@noalloc]
    external to_bytes : bytes -> field_element -> unit @@ portable = "mc_np384_to_bytes" [@@noalloc]
    external from_montgomery : out_field_element -> field_element -> unit @@ portable = "mc_np384_from_montgomery" [@@noalloc]
    external to_montgomery : out_field_element -> field_element -> unit @@ portable = "mc_np384_to_montgomery" [@@noalloc]
  end
...
""
  ...
  module Foreign = struct
    external mul : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_p521_mul" [@@noalloc]
    external sub : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_p521_sub" [@@noalloc]
    external add : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_p521_add" [@@noalloc]
    external to_montgomery : out_field_element -> field_element -> unit @@ portable = "mc_p521_to_montgomery" [@@noalloc]
    external from_octets : out_field_element -> string -> unit @@ portable = "mc_p521_from_bytes" [@@noalloc]
    external set_one : out_field_element -> unit @@ portable = "mc_p521_set_one" [@@noalloc]
    external nz : field_element -> bool @@ portable = "mc_p521_nz" [@@noalloc]
    external sqr : out_field_element -> field_element -> unit @@ portable = "mc_p521_sqr" [@@noalloc]
    external from_montgomery : out_field_element -> field_element -> unit @@ portable = "mc_p521_from_montgomery" [@@noalloc]
    external to_octets : bytes -> field_element -> unit @@ portable = "mc_p521_to_bytes" [@@noalloc]
    external inv : out_field_element -> field_element -> unit @@ portable = "mc_p521_inv" [@@noalloc]
    external select_c : out_field_element -> bool -> field_element -> field_element -> unit @@ portable = "mc_p521_select" [@@noalloc]
    external double_c : out_point -> point -> unit @@ portable = "mc_p521_point_double" [@@noalloc]
    external add_c : out_point -> point -> point -> unit @@ portable = "mc_p521_point_add" [@@noalloc]
    external scalar_mult_base_c : out_point -> string -> unit @@ portable = "mc_p521_scalar_mult_base" [@@noalloc]
  end

  module Foreign_n = struct
    external mul : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_np521_mul" [@@noalloc]
    external add : out_field_element -> field_element -> field_element -> unit @@ portable = "mc_np521_add" [@@noalloc]
    external inv : out_field_element -> field_element -> unit @@ portable = "mc_np521_inv" [@@noalloc]
    external one : out_field_element -> unit @@ portable = "mc_np521_one" [@@noalloc]
    external from_bytes : out_field_element -> string -> unit @@ portable = "mc_np521_from_bytes" [@@noalloc]
    external to_bytes : bytes -> field_element -> unit @@ portable = "mc_np521_to_bytes" [@@noalloc]
    external from_montgomery : out_field_element -> field_element -> unit @@ portable = "mc_np521_from_montgomery" [@@noalloc]
    external to_montgomery : out_field_element -> field_element -> unit @@ portable = "mc_np521_to_montgomery" [@@noalloc]
  end
...
module X25519 = struct
  (* RFC 7748 *)
  external x25519_scalar_mult_generic : bytes -> string -> string -> unit @@ portable = "mc_x25519_scalar_mult_generic" [@@noalloc]
  ...
  let (gen_key_with @ portable) ~g ?compress:_ () =
    let secret = Mirage_crypto_rng.generate_portable ~g key_len in
    secret, public secret
  ...
  let (key_exchange @ portable) secret public =
...
module Ed25519 = struct
  external scalar_mult_base_to_bytes : bytes -> string -> unit @@ portable = "mc_25519_scalar_mult_base" [@@noalloc]
  external reduce_l : bytes -> unit @@ portable = "mc_25519_reduce_l" [@@noalloc]
  external muladd : bytes -> string -> string -> string -> unit @@ portable = "mc_25519_muladd" [@@noalloc]
  external double_scalar_mult : bytes -> string -> string -> string -> bool @@ portable = "mc_25519_double_scalar_mult" [@@noalloc]
  external pub_ok : string -> bool @@ portable = "mc_25519_pub_ok" [@@noalloc]
  ...
  type pub : immutable_data = string
  ...
  let sha512_verify : _ @ portable = fun r key msg ->
    Bytes.of_string (Digestif.digest_string_raw `SHA512 (r ^ key ^ msg))
  ...
  let priv_of_octets : _ @ portable = fun buf ->
  ...
  let verify : _ @ portable = fun ~key signature ~msg ->
    (* section 5.1.7 *)
    if String.length signature = 2 * key_len then
      ...
      if s_smaller_l then begin
        let k = sha512_verify r key msg in
        reduce_l k;
        let k = Bytes.unsafe_to_string k in
