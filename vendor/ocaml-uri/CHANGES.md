...
* **Renamed to `uriz`.** The package is `uriz`, the module is `Uriz` and the
  ...
  replacement; authorship lineage from ocaml-uri is retained.
* **Complete rewrite for the OxCaml compiler.** The library no longer builds
  on stock OCaml. Tested with `ocaml-variants.5.2.0+ox`.
* **Locality.** Every read-only operation accepts its arguments at mode
  `local`; `of_string` and `to_string` are mode-polymorphic through
  `ppx_template`, giving `of_string__local` and `to_string__local`; callback
  parameters such as `query_iter`'s are taken at mode `local`. Every export
  carries the `portable` modality.
* **Region-allocated producers.** `resolve__local` and `normalize__local`
  compose their result text in the caller's region with `Base.Bytes.create_local`
  and freeze it into a local string, so they allocate nothing on the heap at
...
* **`Uriz.t` has the `immutable_data` kind.** A URI is a string and a row of
  offsets into it, never written after construction, so it crosses
  portability and contention. A URI parsed once at module level can be read
  from a portable closure, and so can a `Uriz.t or_null` such as the result
  of `of_string`.
...
* `resolve` now composes into a single scratch buffer instead of a `Buffer`
  plus two per-pass scratches, cutting it from six allocations to three, and
  `normalize` returns its argument unchanged without allocating when the path
  has no dot segments.
* **New `Uriz.of_string_canonical`**: parses onto the stack and rejects input
  that is not already canonical, so the record never reaches the heap and the
  canonical string is the caller's own. Checked `[@zero_alloc]`; a
  parse/inspect/drop cycle allocates 0 bytes.
* The scanner's grammar productions are built from an internal, fully inlined
  combinator layer (`seq`/`alt`/`star`/`count`/`between`/`nfb`); leaves that
  would need backtracking stay hand-rolled. Measured free.
* **Breaking:** the whole URI API is new. A `Uriz.t` is now a canonical
  percent-encoded string plus span indices; `to_string` is free and
  `equal`/`compare`/`hash` are string operations on the canonical form.
* **Breaking:** parsing is strict. `Uriz.of_string` returns `t or_null` and
  rejects anything that is not an RFC 3986 `URI-reference`, instead of
  silently returning an empty URI. The forgiving quirks of 4.x (newline
  truncation, prefix parsing, sloppy IPv4 octets, lenient percent-encodings)
  are gone.
* New `Uriz.Raw` scanner: a single-pass RFC 3986 validator returning an
  unboxed record of span offsets, statically checked `[@zero_alloc]`.
* `of_string` shares the input string when it is already canonical, so a
  parse costs one allocation.
* Dropped the `angstrom` and `stringext` dependencies, and the `uri-re`,
  `uri-sexp`, `uri-bench`, `uri.services` and `uri.services_full` packages.
