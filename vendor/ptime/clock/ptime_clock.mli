...
val current_tz_offset_s : unit -> Ptime.tz_offset_s option @@ nonportable
(** [current_tz_offset_s ()] is the system's current local time zone
    ...
    This is the one function here that cannot be called from a portable
    context. Its stub reads [localtime] and [gmtime], which answer with
    pointers into static storage that every thread shares. *)
