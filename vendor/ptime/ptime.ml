...
module Span = struct
  ...
  (* Upstream indexed a module-level [int64 array] here. An array is mutable
     data, so a portable function cannot read one held at module level. The
     divisors are a match instead. The [invalid_arg] arm reproduces what
     [Array.get] raised on an unclipped index, which [truncate_down] can
     still reach. *)

  let frac_div = function
  | 0 -> 1_000_000_000_000L
  | 1 ->   100_000_000_000L
  | 2 ->    10_000_000_000L
  | 3 ->     1_000_000_000L
  | 4 ->       100_000_000L
...
(* Upstream closed over a module-level [int array] of month lengths here. A
   closure over an array is not portable, so the lengths are a match instead.
   The [invalid_arg] arm reproduces what [Array.get] raised on a month outside
   [1;12], which the only caller rules out beforehand. *)

let max_month_day y m =           (* max day number in a given year's month. *)
  let is_leap_year y = (y mod 4 = 0) && (y mod 100 <> 0 || y mod 400 = 0) in
  if m = 2 then (if is_leap_year y then 29 else 28) else
  match m with
  | 1 | 3 | 5 | 7 | 8 | 10 | 12 -> 31
  | 4 | 6 | 9 | 11 -> 30
  | _ -> invalid_arg "index out of bounds"
...
(* Upstream closed over a module-level array of the seven days here. A match
   takes its place, for the reason given at [max_month_day]. [weekday_num]
   answers in [0;6], so the [invalid_arg] arm is unreachable. *)

let weekday ?tz_offset_s t = match weekday_num ?tz_offset_s t with
| 0 -> `Sun
| 1 -> `Mon
| 2 -> `Tue
| 3 -> `Wed
