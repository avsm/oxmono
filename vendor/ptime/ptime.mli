...
(** The type for signed picosecond precision POSIX time spans. A value
    ...
    timestamps. A span is a pair of numbers, so it crosses portability
    and contention. *)
...
(** The type for picosecond precision POSIX timestamps in the range
    ...
    this type, are by definition always on the UTC timeline. A timestamp
    is a pair of numbers, so it crosses portability and contention. *)
