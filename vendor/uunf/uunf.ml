...
let decomp u =
  ...
  if u < 0xAC00 || 0xD7A3 < u then
    Stdlib_stable.Iarray.to_array (decomp_prop u)
  else
...
let _composite u1 u2 =
  ...
  | [: :] -> ux_none
  | a (* [u2; c; u2'; c'; ...] sorted *) ->
      let len = Stdlib_stable.Iarray.length a / 2 in
      let i = ref 0 in
      try
        while (!i < len) do
          if Stdlib_stable.Iarray.get a (!i * 2) = u2
          then raise Exit else incr i;
        done;
        ux_none
      with Exit -> (Stdlib_stable.Iarray.get a (!i * 2 + 1))
...
let rec add n u =
  if 0xAC00 <= u && u <= 0xD7A3 then
    ...
    | [: :] -> ordered_add n u
    | d ->
        if d_compatibility (Stdlib_stable.Iarray.get d 0) && not n.compat
        then ordered_add n u else
        begin
          add n (_d_uchar (Stdlib_stable.Iarray.get d 0));
          for i = 1 to Stdlib_stable.Iarray.length d - 1 do
            add n (Stdlib_stable.Iarray.get d i)
          done
        end
    end
